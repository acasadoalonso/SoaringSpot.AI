...
4. **Extract the IGC filename and download URL from the result JSON.**
   ...
   - **Download URL** — the flight link under the result's
     `_links` → `http://api.soaringspot.com/rel/flight` → `href`, e.g.
     `http://api.soaringspot.com/v1/flights/10541334728`. This authenticated
     endpoint returns the raw IGC content (`Content-Type:
     application/vnd.flight+igc`).

   Note: do **not** rely on the MCP `get_flight` tool to retrieve the bytes — it
   parses responses as JSON and errors on raw IGC content. Use the helper script
   below instead, which performs the authenticated download directly.
